...
//Services
import {getUserList,getUserPartnerList} from '../../../service/userService'
...
const PartnerListPrime = () => {
    ...
    useEffect(() => {
        ...
        const fetchList = async()=>{
            userList = await getUserPartnerList()
            console.log("Prime List partner",userList)
    ...
    const confirmar = () =>{
        ...
        if (estadoPrincipal == AccionesApp.accionesSocioGarage.accionPrincipal){accionSecundaria(AccionesApp.accionesSocioGarage.busqueda2)}
        if (estadoPrincipal == AccionesApp.accionesCrudUsuarios.accionPrincipal){console.log("Confirmar y volver a main")}

    }

    const SocioVehiculoPaso2 = () =>{
        console.log("Confirmar y seguir")
        if (estadoPrincipal == AccionesApp.accionesSocioVehiculo.accionPrincipal){accionSecundaria(AccionesApp.accionesSocioVehiculo.busqueda2)}
        if (estadoPrincipal == AccionesApp.accionesCrudUsuarios.accionPrincipal){console.log("Confirmar y volver a main")}
        ...
        <div className="datatable-filter-demo">
            <div className="card">
                <DataTable 
                    ...
                    <Column field="dni" header="Dni" filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    ...
                    <Column field="role.description" header="Rol" filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    <Column field="phone" header="Teléfono" filter filterPlaceholder="Search by phone" style={{ minWidth: '12rem' }} />
                ...
                {(estadoPrincipal == AccionesApp.accionesSocioGarage.accionPrincipal
                    ...
                    {(usuarioSeleccionado != null && estadoPrincipal == AccionesApp.accionesSocioGarage.accionPrincipal)
                        ?<Button label="Confirmar" className="p-button-raised p-button-info m-2" onClick={() => confirmar()}/>:null}

                    {(usuarioSeleccionado != null && estadoPrincipal == AccionesApp.accionesSocioVehiculo.accionPrincipal)
                        ?<Button label="Confirmar" className="p-button-raised p-button-info m-2" onClick={() => SocioVehiculoPaso2()}/>:null}
...
export default PartnerListPrime