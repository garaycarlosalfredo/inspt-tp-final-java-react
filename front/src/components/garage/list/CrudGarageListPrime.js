...
//Services
import {getGarageList,deleteGarage} from '../../../service/garageService'
...
const CrudGarageListPrime = () => {
    ...
    const editarGarage= (rowData)=>{
        ...
        "length": 6
        */
        (rowData.vehicleType !== null)?rowData.typeId = rowData.vehicleType.id : rowData.typeId = null
        if(rowData.owner !== null){rowData.ownerDni = rowData.owner.dni }else{rowData.ownerDni = null}
        if(rowData.vehicle!==null){rowData.plate = rowData.vehicle.plate}else{rowData.vehicle = null}
    ...
    const eliminarVehiculo= async()=>{
        console.log("eliminar vehiculo",garageSeleccionado)

        const res = await deleteGarage(garageSeleccionado)
        console.log('res',res)
        ...
        if ( res.status != null ){
            ...
            var result = arrayRemove(customers1, garageSeleccionado);
    ...
    const confirmDeleteProduct = (garage) => {
        garage.editar = false
        console.log('confirmDeleteProduct',garage)
        cargarGarageSeleccionado(garage)
    ...
    return (
        <div className="datatable-filter-demo">
            ...
            <div className="card">
                ...
                { (garageSeleccionado != null && garageSeleccionado.editar === true) ? <GarageDialog></GarageDialog> : null}
                
                <DataTable 
                    value={customers1} 


                    selection={garageSeleccionado} 
                    ...
                    <Column field="zone.letter" header="Zona" filter filterPlaceholder="Search by zone.letter" style={{ minWidth: '12rem' }} />
                    <Column field="owner.dni" header="Dueño del garage" body={garageBodyTemplate} filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    <Column field="owner.dni" header="Vehiculo" body={garageBodyTemplatePlate} filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    <Column field="vehicleType.description" header="vehicleType" filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    <Column field="owner.dni" header="Dni responsable" filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
                    ...
                    {(estadoPrincipal == AccionesApp.accionesCrudGarage.accionPrincipal)?
                    <Column body={actionBodyTemplate} exportable={false} style={{ minWidth: '8rem' }}></Column>:null}
                </DataTable> 
                <div className='d-flex justify-content-center'>
                    {(garageSeleccionado != null && estadoPrincipal !== AccionesApp.accionesCrudGarage.accionPrincipal)
            ...
            <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="confirmation-content">
                    ...
                    {garageSeleccionado && <span>Seguro eliminar <b>patente {garageSeleccionado.plate}</b>?</span>}
                </div>
            </Dialog>
        </div>
    );
